...
![image-20250730095318946](.\imgs_seg\image-20250730095318946.png)

##### 模型框架：

- 输入图片
- 主干网络提取特征
- 经过FPN特征金字塔对不同尺度的特征图进行融合
- 检测分支输出都输出**类别、边框信息（x,y,w,h）、k个mask Coefficients（mask的的置信度**，取值1或-1**）**；
- 分割分支，针对当前输入图片，输出k个Prototype（mask原型图）；不同图片输出的Prototype有差异，但数量也是k个。
- 对于每个目标物体，将k个mask Coefficients（mask的的置信度）与k个Prototype（mask原型图）进行相乘，然后将所有结果相加，得到该目标物体的实例分割结果。

YOLACT将目标检测和实例分割任务并行计算，在实例分割分支生成k个mask原型图（prototype masks），检测分支生成k个mask原型图的置信度（mask coefficients），然后将mask原型图与mask原型置信度进行线性组合，生成实例分割结果

##### prototype masks

- YOLACT 实际上是学习了一种分布表示，在一张图片中可能有多个类别，但不同类别之间的物体共享 prototypes 的组合
- 每一个prototype mask是生成的相对于整张图片的，不是局部的
- prototype masks 的数量与物体类别无关(如，类别数量可能多于 prototypes 的数量

YOLACT 是针对每张图片学习 prototypes，即：不同图片生成的prototypes会有差异

##### 目标检测分支

与yolo的目标检测分支类似，不过在类别、边界框的基础上增加了一个mask的置信度（mask Coefficients），每一个coefficients对应一个prototype

![image-20250730100814821](.\imgs_seg\image-20250730100814821.png)

##### 实例分割分支

从P5特征开始上采样并融合，一直到P3层（因为P3的特征图分辨率高，更好的保留了空间细节信息，同是有一定的语义信息），将P3层的特征送入到分割分支，通过1*1的卷积调整通道数量为k，生成k个prototype，该分支在与检测分支的mask Coefficients线性组合之后才会计算损失。

![image-20250730102001236](.\imgs_seg\\image-20250730102001236.png)

##### 损失

- 类别损失：二分类交叉熵损失
- 边界框回归损失：IOU损失
- mask分类损失：分割结果与真实分割标签逐像素进行二分类交叉熵损失
