...
<img src=".\imgs\RGB.jpg" alt="RGB" style="zoom:50%;" />

- R和G都很大，B很小的时候，就是黄色
- R和B都很大，G很小的时候，是品红色
- B和G都很大，R很小的时候，就是青色

<img src=".\imgs\image-20260309153141525.png" alt="image-20260309153141525" style="zoom:50%;" />

​																				RGB颜色图

（1）差值：

- R-G > 0（R-G的值来定位x轴的数值），越大越偏向正红色，也就是RBG颜色图中，x轴越偏向于正红色，反之越偏向正绿色，色度与y轴无关，此时y由B值来控制，B值越大，饱和度越低，越接近于白色。

- R-B > 0，越大越偏向正红色，也就是RBG颜色图中，x轴越偏向于正红色，反之越偏向正蓝色，与y轴无关，此时y由G值来控制，G值越大，饱和度越低，越接近于白色。

- B-G > 0，越大越偏向正蓝色，也就是RBG颜色图中，x轴越偏向于正蓝色，反之越偏向正绿色，与y轴无关，此时y由B值来控制，B值越大，饱和度越低，越接近于白色。

  直接用差值会受到光照的影响

（2）比值（分母为和）：

- R/(R+G)，值越大，从绿色方向，越往正红色靠近，饱和度越高；值越小，从红色方向越往正绿色靠，饱和度越高。
- R/(R+B)，值越大，从蓝色方向， 越往正红色靠近，饱和度越高；值越小，从红色方向越往正蓝色靠，饱和度越高。
- 其他的依次类推，共有六种组合，都与亮度无关。

（3）分母为差值，比如R/(R-G)，当R与等于G时，会存在除0风险。

（4）差值比和值：

- （R-G）/(R+G)，归一化红绿比，光照影响小。
